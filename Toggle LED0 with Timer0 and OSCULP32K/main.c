 ...
 *
 * Description:
 *		Blink an LED using a timer with frequency control based on OSC32K Clock Source
 *
 *
 * Resources:
 *		SAML21 Datasheet
 ...
 *
 *			Example: (**Count Up**)
 *			Freq = 32768kHz / ([2] * [1] * [64] * [50+1]) = 5.0196 [Hz]   <--- Checked Calc with Fluke 87V, Measured: 5.04 Hz
 *
 * Clock Architecture:
 *		32KHz ULP --> MCLK (IN) --> |DIV: 1| --> MCLK (OUT) --> GCLK0 (IN) --> |DIV: 1| --> GCLK0 (OUT) --> TC0 (IN) --> |DIV: 64|
 *
 * Timer Architecture:
 ...
 *					  Just link the OSCULP32K to a General Clock Gen. (Pg. 251).
 *
 *		OSC32K		- Must be enabled in the Master Clock, then enabled in its own OSC32KCTRL register.
 */
...
int main(void)
{
	...
	*/
	...
	REG_GCLK_GENCTRL2 |= (GCLK_GENCTRL_DIV(1) | GCLK_GENCTRL_GENEN | GCLK_GENCTRL_SRC_OSCULP32K);	// Clock Divide: 1 | Enable Gen 2 (Pg. 135) | Gen0 Source: OSCULP32K (Pg. 135)
